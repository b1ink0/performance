...
window.plvtInitViewTransitions = ( config ) => {
	...
	/**
	 * Determines the view transition type that is active for the given view transition.
	 *
	 * The type is assigned declaratively in CSS, via the `@navigation` rules printed for the current request, so this
	 * only has to find which of the configured animations the browser selected.
	 *
	 * @param {ViewTransition} viewTransition The active view transition.
	 * @return {string} View transition type (e.g. 'default', 'chronological-forwards', 'chronological-backwards').
	 */
	const getActiveTransitionType = ( viewTransition ) => {
		for ( const transitionType of viewTransition.types ) {
			if ( config.animations?.[ transitionType ] ) {
	...
	window.addEventListener(
		...
		( /** @type {PageSwapEvent} */ event ) => {
			if ( event.viewTransition ) {
				...
				const transitionType = getActiveTransitionType(
